...
// Define US states for dropdowns
const US_STATES = [
...
export default function IntakePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    credentialing_type: '', // individual or group
    ...
    try {
      ...
      const { data: client, error: clientError } = await supabase
        ...
        .insert({
          ...
          current_states: [formData.current_operating_state],
          target_states: [formData.target_state],
      ...
      if (client) {
        ...
        if (formData.wants_medicaid === 'yes') {
          payersToInsert.push({
            client_id: client.id,
            payer_name: `${formData.target_state} Medicaid`,
            payer_type: 'medicaid',
            priority: 1
          })
          
          formData.medicaid_mcos.forEach((mco, index) => {
            payersToInsert.push({
              client_id: client.id,
              payer_name: mco,
              payer_type: 'mco',
        ...
        formData.commercial_payers.forEach((payer, index) => {
          payersToInsert.push({
            client_id: client.id,
            payer_name: payer,
            payer_type: 'commercial',
            priority: formData.medicaid_mcos.length + index + 2
          })
        })
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Credentialing Intake Form</h1>
        <p className="mt-2 text-gray-600">
          Complete this comprehensive form to begin your credentialing process
        </p>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="mt-2 flex justify-between">
          {steps.map((step, index) => (
            <div key={index} className="text-xs text-gray-600 text-center max-w-[100px]">
              {step}
            </div>
          ))}
        </div>
          ...
          <div className="space-y-6">
            <div className="rounded-lg bg-white p-6 shadow-lg">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label htmlFor="current_operating_state" className="block text-sm font-medium text-gray-700">
                    What state are you currently operating in and credentialed in? *
                  </label>
                  <select
                    name="current_operating_state"
                    id="current_operating_state"
                    required
                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                    value={formData.current_operating_state}
                ...
                <div>
                  <label htmlFor="target_state" className="block text-sm font-medium text-gray-700">
                    Which state are you expanding to? *
                  </label>
                  <select
                    name="target_state"
                    id="target_state"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Do you already have a business entity registered in the new state? *
                  </label>
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Do you have a physical location/address in the new state? *
                  </label>
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                ...
                {formData.has_physical_location === 'yes' && (
                  <div className="space-y-4">
                    <h4 className="font-medium text-gray-900">New State Address</h4>
                    <div>
          ...
          <div className="space-y-6">
            <div className="rounded-lg bg-white p-6 shadow-lg">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Do you want to be credentialed with Medicaid in {formData.target_state || 'the new state'}? *
                  </label>
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                ...
                {formData.wants_medicaid === 'yes' && formData.target_state && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Which Medicaid MCOs in {formData.target_state}?
                    </label>
                    <p className="text-sm text-gray-600 mb-3">
                      We'll help identify the specific MCOs available in your target state
                    </p>
                    <div className="p-4 bg-gray-50 rounded-md">
                      <p className="text-sm text-gray-700">
                        MCO options will be provided based on your selected state
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <dl className="text-sm text-gray-600 space-y-1">
                    ...
                    <div><span className="font-medium">Current State:</span> {formData.current_operating_state}</div>
                    <div><span className="font-medium">Expanding to:</span> {formData.target_state}</div>
